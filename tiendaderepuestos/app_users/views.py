    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            clients = Client(dni=form.cleaned_data['dni'], 
                            ...
                            user_id=id_user,
                            avatar= 'app_users/avatars/noneavatar.png')
...
def my_profile(request):
    ...
    if request.method == "POST":
        ...
        return redirect("app_users:EditProfile")
    else:
        user_info = Client.objects.raw(f"SELECT * FROM app_users_client WHERE user_id = {user.id}")
        user_info_list = list()
        ...
        return render(request, "app_users/edit_profile.html", {"form": form, "user_info": user_info_list})
...
def pass_change(request):
    ...
    user_info = Client.objects.raw(f"SELECT * FROM app_users_client WHERE user_id = {request.user.id}")
    user_info_list = list()
    for info in user_info:
        user_info_list.append(info.dni)
        user_info_list.append(info.phone)
    ...
    return render(request, "app_users/pass_change.html", {"form": form, "user_info": user_info_list})